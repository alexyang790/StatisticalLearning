...
# Loading Data
train_data <- read.csv("/Users/alexyang/Git/StatisticalLearning/Rproject/donor_accept_train-1.csv")
test_data <- read.csv("/Users/alexyang/Git/StatisticalLearning/Rproject/donor_accept_test.csv")
...
```{r}
# Transforming outcomes to 0 and 1
...
### a.3 Prepping Data Using Recipe
...
train_matrix <- as.matrix(train_processed %>% select(-outcome))
...
# Use cross-validation to find optimal rounds
dtrain <- xgb.DMatrix(data = train_matrix, label = train_labels)
cv_model <- xgb.cv(
  params = xgb_params,
  data = dtrain,
  nrounds = 50, # Change later! 
  nfold = 5,
...
write.csv(submission, "yang_zhe.csv", row.names = FALSE)

# Step 7: Visualize Feature Importance
importance <- xgb.importance(model = xgb_model)
importance_df <- as.data.frame(importance)

ggplot(head(importance_df, 20), aes(x = reorder(Feature, Gain), y = Gain)) +
