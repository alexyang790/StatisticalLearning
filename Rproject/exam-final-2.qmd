...
plot(mclust_mode, what = "uncertainty")
```
:::
...
::: {.callout-note title="Solution"}
![answer for
remedy](/Users/alexyang/Git/StatisticalLearning/Rproject/remedyanswer.png){width=100,
height =1000}
:::
...
We use the formula: threshold = 1/(1 + CFN/CFP)
When CFN = 4*CFP, the optimal threshold is: P(Y=1|X) > 1/(1 + CFN/CFP) = 1/(1 + 4) = 0.2
...
> Based on our analysis of donor acceptance patterns from the data provided. We are ablet to identify a few key factors that strongly impact the surgery outcome. We used a specific technique called "Gain-based Feature Importance". Our model went over one hundred plus features but we have identified the top ten most influential factors in donor acceptance which could be found below. However, these factors shuold be considered correlation instead of direct causes for donor heart transpalnt acceptance as more medical research shuold be invested into find the causal relationships. 
...
create_calibration_data <- function(predictions, labels, n_bins = 10) {
  # Create bins of predicted probabilities
  pred_df <- data.frame(
    pred_prob = predictions,
    actual = labels
  ) %>%
    mutate(bin = cut(pred_prob, breaks = seq(0, 1, length.out = n_bins + 1)))
...
create_calibration_plot <- function(cal_data) {
  ggplot(cal_data, aes(x = mean_pred, y = mean_actual)) +
    # Add diagonal reference line
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray50") +
    # Add calibration points with error bars
...
calculate_brier_score <- function(predictions, labels) {
  mean((predictions - labels)^2)
}

# Assuming we have predictions and actual labels
# You'll need to modify this part based on your data availability
train_predictions <- predict(xgb_model, dtrain)
train_brier <- calculate_brier_score(train_predictions, train_labels)

# Create calibration data
cal_data <- create_calibration_data(train_predictions, train_labels)

# Create and display plots
calibration_plot <- create_calibration_plot(cal_data)

# Create reliability diagram (alternative view)
reliability_plot <- ggplot(cal_data, aes(x = mean_pred)) +
...
grid.arrange(calibration_plot, reliability_plot, ncol = 2)

# Print calibration metrics
cat("\nCalibration Metrics:\n")
cat("Brier Score:", round(train_brier, 4), "\n")

# Calculate and print mean calibration error
mean_cal_error <- mean(abs(cal_data$mean_pred - cal_data$mean_actual))
